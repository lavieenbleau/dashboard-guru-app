...
class QuizActivityService
{
    ...
    public function logActivity($studentId, $exerciseId, $eventType, $deviceInfo = null, $ipAddress = null)
    {
        try {
            ...
            if (in_array($eventType, ['QUIZ_EXIT', 'TAB_SWITCH', 'WINDOW_BLUR', 'APP_BACKGROUND'])) {
                $totalExits = QuizActivityLog::where('student_id', $studentId)
                    ->where('exercise_id', $exerciseId)
                    ->whereIn('event_type', ['QUIZ_EXIT', 'TAB_SWITCH', 'WINDOW_BLUR', 'APP_BACKGROUND'])
                    ->count();
            ...
            if (in_array($eventType, ['QUIZ_REJOIN', 'WINDOW_FOCUS', 'APP_RESUME'])) {
                if ($lastLog && in_array($lastLog->event_type, ['QUIZ_EXIT', 'TAB_SWITCH', 'WINDOW_BLUR', 'APP_BACKGROUND'])) {
                    // Calculate total duration from previous exit events
                    $previousAwayTime = QuizActivityLog::where('student_id', $studentId)
                        ->where('exercise_id', $exerciseId)
                        ->whereIn('event_type', ['QUIZ_REJOIN', 'WINDOW_FOCUS', 'APP_RESUME'])
            ...
            if (in_array($eventType, ['QUIZ_ENTER', 'START'])) {
                if ($lastLog && !in_array($lastLog->event_type, ['QUIZ_EXIT', 'QUIZ_SUBMIT', 'SUBMIT'])) {
                    $suspiciousFlag = true;
                }
            }
            ...
            \Illuminate\Support\Facades\Log::error('Gagal mencatat log kuis: ' . $e->getMessage());
            return null;
        }
    }
}
