                ...
                <div class="card-body">
                    <form action="{{ route('guru.materi.update', [$serial->id, $tema->id, $subtema->id, $post->id]) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" rows="4" class="form-control @error('description') is-invalid @enderror">{{ old('description', $post->description) }}</textarea>
                            @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="url" name="link" class="form-control @error('link') is-invalid @enderror" value="{{ old('link', $post->link) }}" placeholder="https://example.com">
                            @error('link')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="alert alert-info">
                                ...
                                <a href="{{ Storage::url($post->attachment) }}" target="_blank">{{ basename($post->attachment) }}</a>
                            </div>
                        </div>
                        @endif
                        ...
                        <!-- New Attachment -->
                        <div class="mb-3">
                            <label class="form-label">{{ $post->attachment ? 'Ganti File' : 'File Lampiran' }}</label>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="embed" rows="3" class="form-control @error('embed') is-invalid @enderror">{{ old('embed', $post->embed) }}</textarea>
                            @error('embed')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
