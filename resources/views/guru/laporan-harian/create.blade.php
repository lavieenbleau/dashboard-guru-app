...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0"><i class='bx bx-plus text-primary me-2'></i>Tambah Aktivitas Siswa</h5>
        </div>
        <div class="card-body">
            <form action="{{ route('guru.laporanharian.store', $serial->id) }}" method="POST">
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <textarea name="activity" class="form-control @error('activity') is-invalid @enderror" 
                              rows="4" required placeholder="Deskripsikan aktivitas siswa...">{{ old('activity') }}</textarea>
                    @error('activity')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
