...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <h4 class="mb-0 fw-bold text-dark"><i class='bx bx-edit text-warning me-2'></i>Edit Tugas</h4>
    </div>
                    ...
                    <div class="card-body p-4">
                        <div class="mb-4">
                            ...
                            <input type="text" name="title" class="form-control form-control-lg @error('title') is-invalid @enderror" 
                                   value="{{ old('title', $task->title) }}" placeholder="Contoh: Laporan Praktikum Perubahan Wujud Benda" required>
                            @error('title')
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" class="form-control summernote @error('description') is-invalid @enderror">{{ old('description', $task->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-2">
                            ...
                            <!-- Hidden Checkbox for removing existing attachment -->
                            <input type="checkbox" name="remove_attachment" id="removeAttachment" value="1" class="d-none">

                            <!-- Area Drop Zone -->
                            <div class="drop-zone" id="dropZone" style="display: {{ $task->attachment ? 'none' : 'block' }};">
                            ...
                            <!-- Area Preview File -->
                            <div class="file-preview mt-3" id="filePreview" style="display: {{ $task->attachment ? 'block' : 'none' }};">
                                <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
                                    <div class="d-flex align-items-center">
                                        <i class='bx bxs-file-blank file-icon me-3'></i>
                                        <div>
                                            <h6 class="mb-0 text-dark" id="fileName">
                                                @if($task->attachment)
                                                    <a href="{{ asset('storage/' . $task->attachment) }}" target="_blank" class="text-decoration-none">{{ basename($task->attachment) }}</a>
                                                @else
                                                    nama_file.pdf
                                                @endif
                                            </h6>
                                            <small class="text-muted" id="fileSize">{{ $task->attachment ? 'File Saat Ini' : '2.3 MB' }}</small>
                                        </div>
                                    </div>
                    ...
                    <div class="card-body p-4">
                            ...
                            <div class="list-group @error('classroom_ids') is-invalid border-danger @enderror" style="max-height: 250px; overflow-y: auto; border-radius: 8px;">
                                @forelse($classrooms as $classroom)
                                    <label class="list-group-item d-flex gap-2 px-3 py-2 cursor-pointer border-1">
                                        <input class="form-check-input classroom-checkbox mt-0" type="checkbox" name="classroom_ids[]" value="{{ $classroom->id }}" 
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group input-group-merge">
                                ...
                                <input type="url" name="link" class="form-control @error('link') is-invalid @enderror" value="{{ old('link', $task->link) }}" placeholder="https://...">
                            </div>
                            @error('link')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
            ...
            <div class="col-12 mt-2">
                <div class="card card-modern bg-transparent border-0 shadow-none">
                    <div class="card-body p-0 d-flex justify-content-end gap-3 flex-wrap">
                        ...
                        <button type="submit" class="btn btn-warning btn-lg px-5 shadow-sm fw-bold text-white">
                            <i class='bx bx-save me-2'></i>Update Tugas
                        </button>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Store original markup for reset
    const originalFileNameHtml = fileName ? fileName.innerHTML : '';
    ...
    if (dropZone && fileInput) {
        ...
        removeFileBtn.addEventListener('click', () => {
            ...
            // Mark existing attachment for removal on server
            if (removeAttachmentInput) {
                removeAttachmentInput.checked = true;
            }
            // Reset to original if needed later? No, keep it cleared.
        });
        ...
        function handleFiles() {
            if (fileInput.files.length > 0) {
                ...
                // Uncheck remove existing because we are replacing it
                if (removeAttachmentInput) {
